...
/**
 * 將對方 APP 的類別名稱對應到本 app 的類別 key
 */
function mapSourceCategoryToKey(type: TransactionType, categoryName: string): string {
...
export function parsedRowsToTransactions(
  ...
  for (const row of rows) {
    const categoryKey = mapSourceCategoryToKey(row.type, row.categoryName);
